...
select * , 2023 - year(ngay_sinh) from khach_hang where (dia_chi like '%Đà Nẵng%' or dia_chi like '%Quảng Trị%') and ( 2023 - year(ngay_sinh) between 18 and 50) ;
...
select khach_hang.ho_ten from khach_hang group by khach_hang.ho_ten having count(*) = 1;

-- 9.	Thực hiện thống kê doanh thu theo tháng, nghĩa là tương ứng với mỗi tháng trong năm 2021 thì sẽ có bao nhiêu khách hàng thực hiện đặt phòng.
select month(ngay_lam_hop_dong) as thang, count(1) as so_luong_khach_hang from hop_dong
where year(ngay_lam_hop_dong) = 2021
group by thang;
...
select dich_vu_di_kem.ma_dich_vu_di_kem, dich_vu_di_kem.ten_dich_vu_di_kem from khach_hang 
...
select hop_dong.ma_hop_dong,nhan_vien.ho_ten, khach_hang.ho_ten, khach_hang.so_dien_thoai, dich_vu.ten_dich_vu, sum(hop_dong_chi_tiet.so_luong) as so_luong_dich_vu_di_kem, hop_dong.tien_dat_coc,hop_dong.ngay_lam_hop_dong from hop_dong
join nhan_vien on nhan_vien.ma_nhan_vien = hop_dong.ma_nhan_vien
join khach_hang on khach_hang.ma_khach_hang = hop_dong.ma_khach_hang
join dich_vu on hop_dong.ma_dich_vu = dich_vu.ma_dich_vu
left join hop_dong_chi_tiet on hop_dong.ma_hop_dong = hop_dong_chi_tiet.ma_hop_dong
where hop_dong.ma_hop_dong in (select ma_hop_dong from hop_dong where hop_dong.ngay_lam_hop_dong between '2020-10-31' and '2020-12-31' ) and
hop_dong.ma_hop_dong not in (select ma_hop_dong from hop_dong where hop_dong.ngay_lam_hop_dong between '2021-01-01' and '2021-06-31' )
group by hop_dong.ma_hop_dong ;
...
select ma_hop_dong.ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, count(1) as so_lan_su_dung
from hop_dong 
