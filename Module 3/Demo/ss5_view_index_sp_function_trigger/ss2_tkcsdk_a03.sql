...
select s.id, s.name, c.name as class_name, s.point
from student s
         join class c on s.class_id = c.id delimiter //
...
select s.name, s.point, check_point(s.point) as pass_fail
from student s;
